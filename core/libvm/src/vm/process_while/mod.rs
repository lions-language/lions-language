...
impl VirtualMachine {
    pub fn process_while_stmt(&mut self, value: WhileStmt) -> ExecuteResult {
        ...
        loop {
            let (cond_res, exe_res) = self.condition_handle(&expr_addr, &true_handle);
        ...
        match boolean_value.value_ref() {
            BooleanValue::True => {
                ...
                return ExecuteResult::Jump(true_handle.jump_clone());
            },
            BooleanValue::False => {
            }
        }
    ...
    fn condition_handle(&mut self, expr_addr: &AddressValue
