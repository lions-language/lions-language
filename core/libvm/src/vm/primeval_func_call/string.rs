...
impl VirtualMachine {
    ...
    pub fn create_ref_str_plus_operator_ref_str(&mut self, value: CallPrimevalFunction) {
        /*
         * 加载参数
         *  在进入这里的时候, 当前作用域中的映射集合中已经存在了参数的映射
         *  所以, 直接用 索引0访问查找第一个参数的数据, 以此类推 注意:
    ...
    pub fn mut_ref_str_plus_operator_move_str(&mut self, value: CallPrimevalFunction) {
        /*
         * 加载参数
         *  在进入这里的时候, 当前作用域中的映射集合中已经存在了参数的映射
         *  所以, 直接用 索引0访问查找第一个参数的数据, 以此类推 注意:
        ...
        let mut left_value = self.thread_context.current_unchecked().get_data_unchecked(
            &left_param_compile_addr, &self.link_static);
        let right_value = self.thread_context.current_mut_unchecked().take_data_unchecked(
            &right_param_compile_addr, &self.link_static);
        let left_value = extract_primeval_utf8_str_mut!(left_value, Str);
        let right_value = extract_primeval_utf8_str_move!(right_value, Str);
        // println!("{}", &right_value);
        /*
         * 计算返回值
         * */
