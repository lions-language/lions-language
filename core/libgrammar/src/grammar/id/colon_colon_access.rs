...
use crate::grammar::{GrammarParser, Grammar
    ...
    , DescContext, EnterColonColonAccessContext};
use crate::lexical::{CallbackReturnStatus};
use crate::token::{TokenType, TokenData};

impl<'a, T: FnMut() -> CallbackReturnStatus, CB: Grammar> GrammarParser<'a, T, CB> {
    pub fn id_process_coloncolon_access(&mut self, mut desc_ctx: DescContext) {
        /*
         ...
         * */
        self.skip_next_one();
        self.cb().enter_colon_colon_access(EnterColonColonAccessContext::new_with_all(
                module_prefix.clone()));
        self.id_process_inner(desc_ctx, Some(module_prefix));
        self.cb().leave_colon_colon_access();
    }
}
