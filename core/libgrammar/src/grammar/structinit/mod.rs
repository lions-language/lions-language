...
impl<'a, T: FnMut() -> CallbackReturnStatus, CB: Grammar> GrammarParser<'a, T, CB> {
    pub fn structinit_process(&mut self, backtrack_len: usize
        ...
        let mut init_context = StructInitContext::new_with_all(
            struct_name, HeapPtr::new_null(), DescContext::default(), PackageStr::Empty, String::new());
    ...
    fn structinit_single_field(&mut self
         ...
         * expr
         * */
        if let DescResult::Error(e) = self.cb().struct_init_field_before_expr(init_context
            , field_conext) {
            self.panic(&e);
        };
